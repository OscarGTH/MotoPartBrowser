...
# Custom imports
from constants import IGNORED_LINK_URLS, top_motorcycle_brands, YEAR_INSERT_QUERY, MODEL_INSERT_QUERY, BRAND_INSERT_QUERY
...
class Crawler():
    ...
    def parse_main_page(self, page):
        ...
        filtered_links = [link for link in links if link.get('href') not in IGNORED_LINK_URLS]
        for link in filtered_links:
            ...
            for brand in top_motorcycle_brands:
                ...
                if brand.lower() in listing_text.lower():
                    ...
                    listing_without_brand = listing_text.replace(brand, '').replace('-&gt', '')
                    ...
                    if years := self.extract_year(listing_without_brand):
                        ...
                        motorcycle["model"] = listing_without_brand[:years["year_index"]].strip()
    ...
    def get_page(self, page_url):
        try:
            ...
            if response.status_code == 200:
                ...
                logger.error("Failed to retrieve the web page. Status code:", response.status_code)
                return None
    ...
    def parse_part_page(self, page):
        filtered_tables = [
            table
            for table in page.find_all("table", {"align": False, "class": False})
        ]
        ...
        for table in filtered_tables:
            ...
            if len(first_col) == 3:
                ...
                part["name"] = part_name_cell.text.strip() if part_name_cell.text else "Unnamed part"
    ...
    def insert_motorcycles_to_database(self):
        ...
        try:
            with self.connection.cursor() as cursor:
                for motorcycle in self.motorcycles[0:10]:
                    # Adding brand and models to database
                    cursor.execute(BRAND_INSERT_QUERY, (motorcycle.get("brand"),))
                    cursor.execute(MODEL_INSERT_QUERY, (motorcycle.get("model"), motorcycle.get("brand")))
                    # Commiting the brand and model insertions
                    self.connection.commit()
                    # Getting model id from database in order to add year entries
                    cursor.execute("SELECT model_id FROM Models WHERE model_name = %s AND brand_name = %s;", (motorcycle.get("model"), motorcycle.get("brand")))
                    ...
                    if model_id:
                        ...
                        if len(years) > 1:
                            ...
                            for year in range(years[0], year[1] + 1):
                                # Adding each year separately.
                                cursor.execute(YEAR_INSERT_QUERY, (model_id[0], year, motorcycle.get("href")))
                        else:
                            cursor.execute(YEAR_INSERT_QUERY, (model_id[0], years[0], motorcycle.get("href")))
        except (Exception, psycopg2.Error) as error:
    ...
    def extract_year(self, listing_text) -> dict:
        ...
        if index_of_year_range > 4 and len(listing_text) >= index_of_year_range + 5:
            ...
            if self.is_valid_year(left_of_hyphen) and self.is_valid_year(right_of_hyphen):
